...
- `src/gfw_fishing_osw_ais/config.py` — paths, stage windows, gear map, CRS, `RANDOM_SEED = 42`
- `src/gfw_fishing_osw_ais/io.py` — `load_grid()`, `load_stage()`, `load_owf()`, `load_aoi()`
- `src/gfw_fishing_osw_ais/explore.py` — Phase 1 distribution diagnostics
- `src/gfw_fishing_osw_ais/aggregate.py` — spatial join + per-cell rollup
- `src/gfw_fishing_osw_ais/weights.py` — lattice-derived Queen contiguity
- `src/gfw_fishing_osw_ais/spatial_stats.py` — Moran / Gi\* / Local Moran's
- `src/gfw_fishing_osw_ais/transitions.py` — stage-change analysis
- `src/gfw_fishing_osw_ais/viz.py` — maps and figure assembly
- `reports/_quarto.yml`
